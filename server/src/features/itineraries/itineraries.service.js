...
export async function getItineraryById(itineraryId) {
  ...
  const allItinerarySlots = await db
  ...
  const slotData = allItinerarySlots
    .map((slot) => {
...
export async function copyItineraryToNew(itineraryID, itineraryName, startDate) {
  try {
    ...
    const sourceItinerary = await db
      .collection("itineraries")
    ...
    const tripDuration = getDaysDifference(sourceItinerary.startDate, sourceItinerary.endDate);
    ...
    itinerarySlots.forEach((slot) => {
      slotID += 1;
      const formattedID = slotID.toString().padStart(3, "0");
      const newSlotDate = getDateOffset(slot.slotDate, sourceItinerary.startDate, newStartDate);
